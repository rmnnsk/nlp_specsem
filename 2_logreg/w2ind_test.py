...
def gen_w2ind(tokenized_texts, min_tf=1, max_df=1, min_df=1, bigrams=False, trigrams=False, quadgrams=False):
    ...
    for text in tokenized_texts:
        ...
        if quadgrams:
            tokens += list(zip(text[:-3], text[1:-2], text[2:-1], text[3:]))
        for token in tokens:
            all_cnt[token] += 1
...
def vectorize(tokenized_texts, w2ind, idfs, mode='tf-idf', bigrams=False, trigrams=False, quadgrams=False, intercept=True):
    start = time.perf_counter()
...
def train(texts, labels):
    ...
    w2ind_train, idf = gen_w2ind(token_train, max_df = 0.4,  min_df=2, **vectorization_params)

    X_train = vectorize(token_train, w2ind=w2ind_train, idfs=idf, mode='default', **vectorization_params)
    print(X_train.shape)

    model = LogisticRegression(num_ep=50, lr=0.001, l2_reg=3e-5, batch_size=1000, optimizer='Adam')

    return model.fit(X_train, y_train), w2ind_train, idf, vectorization_params


def classify(texts, params):
    model, w2ind_train, idf, vec_params = params

    proc_test = list(map(preprocessing, texts))
    token_test = tokenize_dataset(proc_test)

    X_test = vectorize(token_test, w2ind=w2ind_train, idfs=idf, mode='default', **vec_params)
...
if __name__ == '__main__':
    ...
    proc_test = list(map(preprocessing, val_data))
    token_test = tokenize_dataset(proc_test)

    # with open("wrong_pred.txt", 'w', encoding='utf-8') as inp:
    #     for ind, (pred, true) in enumerate(zip(my_labels, val_labels)):
    #         if pred != true:
    #             inp.write(' '.join(token_test[ind]) + '\n')