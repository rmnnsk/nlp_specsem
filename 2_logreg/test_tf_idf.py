...
def gen_w2ind(tokenized_texts, min_tf=1, max_df=0.8, bigrams=False, trigrams=False):
...
def vectorize(tokenized_texts, w2ind, bigrams=False, trigrams=False, intercept=True):
    ...
    for ind, text in enumerate(tokenized_texts):
        if intercept:
            X[ind, 0] = 1
    X = X.tocsr()
    lens = np.array([len(text) for text in tokenized_texts])
...
class LogisticRegression:
    ...
    def predict(self, X):
        y_pred = sigmoid(X.dot(self.weights))
        y_lab = (y_pred > 0.45).astype(np.int32)
        return y_lab
...
def train(texts, labels):
    ...
    model = LogisticRegression(num_ep=50, lr=0.001, l2_reg=1e-4, batch_size=1000, optimizer='Adam')
